...
def create_periodic_table(elements):
    ...
    for index, (symbol, chinese_name, electron_config, row, col) in enumerate(elements, start=1):
        ...
        atomic_number = f"<div style='font-size: 8px; position: absolute; top: 2px; left: 2px;'>{index}</div>"
        colored_symbol = f"<div style='font-size: 18px; font-weight: bold; margin-bottom: -1px;'>{symbol}</div>"
...
with open("periodic_table.md", "w", encoding="utf-8") as file:
    ...
    #支援碰到鼠標後放大
    file.write("td > div:hover { transform: scale(1.15); box-shadow: 0 2px 6px rgba(0,0,0,0.2); z-index: 1; }\n")
    file.write("</style>\n\n")
